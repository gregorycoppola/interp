...
a short description of where it stands.
    ...
    ✅  = fully covered by the BP framework
    ⚠️  = partially covered, extension needed
    ❌  = genuinely open, no clean BP account
...
## Summary
    ...
    ✅  5 types  (previous token, induction, name mover,
                  duplicate token, retrieval)
    ⚠️  3 types  (S-inhibition, positional, redundant)
    ❌  1 type   (function vector)

The ✅ types are all sharp routing — one neighbor,
one value, focused lookup. The theory was built for this
and covers it completely.

The ⚠️ types need formal development but not new
theory. Each is a known extension of existing machinery.

The ❌ type is the genuine gap. Function vector heads
